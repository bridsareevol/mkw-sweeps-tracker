...
const BUILDINGS = {
  Keyes: {
    basementRooms: ['Piano Room B116', 'Laundry Room']
  },
  Matthews: {
    basementRooms: [
      'Study Room B143/A',
...
function moveBuildingChoice(building, direction) {
  const container = document.getElementById('buildingChoices');
  if (!container) {
    return;
  }

  const choices = [...container.querySelectorAll('.building-choice')];
  const currentIndex = choices.findIndex(option => option.dataset.building === building);
  ...
  const [selectedChoice] = choices.splice(currentIndex, 1);
  choices.splice(nextIndex, 0, selectedChoice);
...
function populateBuildingChoices() {
  ...
  Object.keys(BUILDINGS).forEach(building => {
    ...
    label.dataset.building = building;
    label.innerHTML = `
      <input type="checkbox" value="${building}">
      <span>${getBuildingLabel(building)}</span>
      <span class="building-choice-actions">
        <button type="button" class="move-building-button" data-direction="-1" aria-label="Move ${getBuildingLabel(building)} up">↑</button>
        <button type="button" class="move-building-button" data-direction="1" aria-label="Move ${getBuildingLabel(building)} down">↓</button>
      </span>`;

    label.querySelectorAll('.move-building-button').forEach(button => {
      button.addEventListener('click', () => moveBuildingChoice(building, Number(button.dataset.direction)));
    });
...
function summarizeRoomInputs(inputs, quietText = 'Quiet') {
  const statuses = [...inputs]
    .map(formatRoomStatus)
    .filter(Boolean);

  return statuses.length ? statuses.join(' ') : quietText;
}
...
function renderKeyesBuildingFields(fieldsContainer) {
  ...
  fieldsContainer.innerHTML = `
    ...
    ${stepper(2, ['Meeting Room', 'Study Room', 'Games Room'])}
...
function renderMatthewsBuildingFields(fieldsContainer) {
  fieldsContainer.innerHTML = `
    ...
    ${noteField('floor1')}
    <div class="room-steppers matthews-floor1-rooms">
...
function getKeyesBuildingReport(section) {
  ...
  const floors = [6, 5, 4, 3, 2, 1].map(floor => {
    ...
    const roomSummary = summarizeRoomInputs(roomSteppers.querySelectorAll('input[data-room]'));
    const floorLine = floor === 6
      ? (roomSummary === 'Quiet' ? 'Quiet. Roof access locked.' : `${roomSummary} Roof access locked.`)
      : roomSummary;

    return `Floor ${floor}: ${floorLine}${getLocationNote(section, `floor${floor}`)}`;
  }).join('\n');

  const basement = summarizeRoomInputs(section.querySelectorAll('.keyes-basement-rooms input[data-room]'));
  return `Building: Mary E. Keyes\n${floors}\nBasement: ${basement}${getLocationNote(section, 'basement')}`;
}

function getMatthewsBuildingReport(section) {
  const floor1Inputs = [...section.querySelectorAll('.matthews-floor1-rooms input[data-room]')];
  const floor1Status = summarizeRoomInputs(floor1Inputs, section.querySelector('[name="floor1"]').value.trim() || 'Quiet');

  const floors = [4, 3, 2, 1].map(floor => {
    const value = floor === 1
      ? floor1Status
      : section.querySelector(`[name="floor${floor}"]`).value.trim() || 'Quiet';
    return `Floor ${floor}: ${value}${getLocationNote(section, `floor${floor}`)}`;
  }).join('\n');

  const basement = summarizeRoomInputs(section.querySelectorAll('.matthews-basement-rooms input[data-room]'));
...
function getWallingfordBuildingReport(section) {
  ...
  const floor1 = summarizeRoomInputs([teaRoom]);
  const basement = summarizeRoomInputs(section.querySelectorAll('.wallingford-basement-rooms input[data-room]'));
